...
{
        int64_t nSubsidy = 0 * COIN; //Premine
    
    if(pindexBest->nHeight+1 <=100)
    {
        nSubsidy = 918000 * COIN;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "QCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "21 Jan 2015 bullishbeeR comes with us officially!";
        CTransaction txNew;
        txNew.nTime = 1421859937;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1421859937;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 2617753 : 21373;
...
unsigned char pchMessageStart[4] = { 0x37, 0x09, 0x14, 0x15 };
